...
-- The style is configurable through a config file named 
-- "lua-settings/stats.conf"
...
function add_file(s)
    ...
    if append_property(s, "file", "cache-used", "Cache:") then
        append_property_inline(s, "file", "demuxer-cache-duration", "+", " sec", 
true, true)
    end
...
function add_video(s)
    ...
    if append_property(s, "video", "video-params/w", "Native Resolution:") then
        append_property_inline(s, "video", "video-params/h", " x ", "", true, 
true, true)
    end
...
function add_header(s)
    if o.custom_header and o.custom_header ~= "" then
        ...
        s.header = 
string.format([[%s{\\fs%d}{\\fn%s}{\\bord%f}{\\3c&H%s&}{\\1c&H%s&}
...
function append_property(s, sec, prop, prefix, suffix)
    ...
    s[sec] = s[sec] .. o.nl .. o.prop_indent .. b(desc) .. o.kv_sep .. 
no_ASS(ret) .. suf
    return true
end
